  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white">Gestión de Ausencias / Licencias</h1>
      ...
      <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-5 shadow-lg">
        <form method="GET" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 text-white pl-9 pr-4 py-2 rounded-xl text-xs outline-none transition-all"
              />
            </div>
          </div>
          ...
          {/* Departamento */}
          <div className="space-y-1">
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Departamento</label>
            <select
              name="dept"
          ...
          {/* Tipo de Ausencia */}
          <div className="space-y-1">
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Tipo de Ausencia</label>
            <select
              name="type"
          ...
          {/* Certificado médico */}
          <div className="space-y-1">
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Certificado Médico</label>
            <select
              name="cert"
      ...
      <div className="bg-slate-900 border border-slate-800/80 rounded-2xl overflow-hidden shadow-xl">
        {absences.length === 0 ? (
          <div className="py-16 text-center text-slate-500 flex flex-col items-center justify-center">
            ...
            <p className="text-sm font-semibold">No se encontraron registros de ausencias</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/50 text-slate-300">
                {absences.map((ab) => (
                  <tr key={ab.id} className="hover:bg-slate-850/20 transition-colors group">
                    {/* Empleado */}
                    <td className="py-4 px-6">
                      <div className="font-semibold text-white group-hover:text-indigo-400 transition-colors">
                        {ab.last_name}, {ab.first_name}
                      </div>
                      <div className="text-[10px] text-slate-550 mt-0.5 font-mono">{ab.file_number}</div>
                    </td>
                    {/* Departamento */}
                    <td className="py-4 px-6 text-slate-400">{ab.department_name}</td>
                    {/* Motivo */}
                    <td className="py-4 px-6">
                      <div>
                        <span className="font-medium text-slate-200">{ab.absence_type_name}</span>
                        {ab.reason && (
                          <p className="text-[10px] text-slate-500 mt-1 truncate max-w-xs" title={ab.reason}>
                            "{ab.reason}"
                          </p>
                        )}
                      </div>
                    ...
                    <td className="py-4 px-6 text-center">
                      {!ab.requires_certificate ? (
                        <span className="text-[10px] text-slate-500 bg-slate-950 px-2 py-0.5 rounded border border-slate-800 font-medium">No Requiere</span>
                      ) : ab.certificate_attached ? (
                        <span className="inline-flex items-center gap-1 text-[10px] text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-full border border-emerald-500/20 font-medium">
                          <CheckCircle className="w-3 h-3" />
                          Entregado
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 text-[10px] text-rose-400 bg-rose-500/10 px-2.5 py-0.5 rounded-full border border-rose-500/20 font-medium">
